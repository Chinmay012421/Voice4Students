...
def init_db():
    ...
    # Safe migrations for databases created by older SchoolFix versions.
    ...
    if "registered_at" not in user_columns:
        cur.execute("ALTER TABLE users ADD COLUMN registered_at TEXT NOT NULL DEFAULT ''")
        cur.execute("UPDATE users SET registered_at=? WHERE registered_at=''",
...
def save_user(username, password, role="student"):
    conn = connect()
    conn.execute("INSERT INTO users (username,password,role,must_change_password,registered_at) VALUES (?,?,?,?,?)",
...
def get_all_users():
    conn = connect()
    rows = conn.execute(
        "SELECT id, username, role, active, must_change_password, registered_at "
...
def set_temporary_password(user_id, temporary_password):
    ...
    if row[2] == "main_admin":
        conn.close()
        return "protected"
    cur.execute("UPDATE users SET password=?, must_change_password=1 WHERE id=?",
...
def must_change_password(username):
    conn = connect()
    row = conn.execute("SELECT must_change_password FROM users WHERE username=? AND active=1",
                       (username,)).fetchone()
    conn.close()
